...
"""Calendar platform for BEE Trash Pickup."""
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities = [
        BEETrashCalendar(coordinator, abfallart)
        for abfallart in ABFALLARTEN
    ]
    async_add_entities(entities)


class BEETrashCalendar(CalendarEntity):
    ...
    def event(self):
        ...
        if not upcoming:
            return None
        d = upcoming[0]
        summary = f"{self.abfallart} ({self.coordinator.entry.title})"
    ...
    async def async_get_events(self, hass, start_date, end_date):
        ...
        for d in self._get_dates():
            if start <= d < end:
                events.append(CalendarEvent(start=d, end=d + timedelta(days=1), summary=summary))
        return events
